...
resource "azurerm_public_ip" "region1-vpn01-pip" {
  name                = "r1-vpn01-pip"
  resource_group_name = azurerm_resource_group.rg1.name
  location            = var.loc1
  allocation_method   = "Static"
...
resource "azurerm_virtual_network_gateway" "region1-vpn01" {
  name                = "r1-vpn01"
  resource_group_name = azurerm_resource_group.rg1.name
  location            = var.loc1
  ...
  ip_configuration {
    ...
    public_ip_address_id          = azurerm_public_ip.region1-vpn01-pip.id
    private_ip_address_allocation = "Dynamic"
    subnet_id                     = azurerm_subnet.region1-vnet1-snet3.id
  }
